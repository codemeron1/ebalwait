    ...
    <div className="h-screen flex items-start md:items-center bg-background relative overflow-hidden">
      ...
      >
        <div className="w-full md:max-w-5xl h-full bg-background">
          {/* Content Card */}
          <div
            className="h-full bg-background backdrop-blur-sm rounded-none md:rounded-md 
            ...
            <div className="flex-1 p-0 overflow-y-auto min-h-0">
              <div className="prose prose-slate max-w-none h-full">
                <div className="relative h-full flex flex-col">
                  ...
                  <div
                    className="flex-1 bg-slate-50 rounded-none md:rounded-lg md:shadow-2xl 
                                        border border-slate-300 overflow-hidden flex flex-col"
                  >
                    {/* Window Title Bar */}
                    <div
                      className="bg-linear-to-r from-slate-700 to-slate-800 px-4 py-2 flex items-center justify-between 
                    ...
                    <div className="relative flex-1 flex flex-col bg-background">
                      {/* Application Header */}
                      <header
                        className="bg-slate-100 border-b border-slate-200 px-6 py-4 
                      ...
                      {/* Main Content Area */}
                      <main
                        className="relative bg-background overflow-y-auto max-h-[70vh]
                      ...
                      {/* Status Bar */}
                      <div
                        className=" fixed bottom-0 left-0 right-0 bg-slate-200 border-t border-slate-300 
                      ...
                      >
                        <div className="flex items-center gap-4">
                          ...
                          <span className="flex items-center gap-1">
                            ...
                            } ${user?.last_name || "Dela Cruz"}`}</span>{" "}
                            |
                            <span className="px-2">
                              {roles.current[Number(user?.role) - 1 || 0] ||
                                "Team Member"}
                            </span>
                          </span>
                        </div>
            ...
            {menuItems.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`group cursor-pointer relative w-14 h-14 rounded-xl flex items-center justify-center transition-all duration-300 ${
                    isActive
                      ? " bg-slate-500 to-purple-600 text-white shadow-lg scale-110 shadow-blue-500/25"
                      : "text-slate-600 hover:bg-slate-100 hover:scale-105 hover:shadow-md"
                  }`
                }
                ...
                >
                  ...
                  {/* Active Indicator */}
                  {isActive && (
                    <div className="absolute -left-1 top-1/2 -translate-y-1/2 w-1 h-8 bg-linear-to-b 
                      bg-slate-600 rounded-full"></div>
                  )}
            ...
            {menuItems.map((item) => {
              ...
              return (
                <button
                  key={item.id}
                  onClick={() => setActiveMenu(item.id)}
                  className={`group relative w-12 h-12 rounded-xl flex items-center 
                                      justify-center transition-all duration-300 
                                      ${
                                        isActive
                                          ? ` bg-slate-500  text-white 
                                            shadow-lg scale-110 shadow-blue-500/25`
